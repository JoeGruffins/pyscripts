...
<html lang="en">
<head>
    <title>Code coverage report for build-tx/transaction.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">All files</a> / <a href="index.html">build-tx</a> transaction.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
function <span class="fstat-no" title="function not covered" >outputComparator(</span>aScript: Buffer, aValue: string, bScript: Buffer, bValue: string) {
<span class="cstat-no" title="statement not covered" >    return new BigInteger(aValue).compareTo(new BigInteger(bValue)) || aScript.compare(bScript);</span>
}
&nbsp;
...
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if ((!isChange) &amp;&amp; original.type === 'opreturn') {
        ...
        // $FlowIssue
        const address = isChange ? <span class="branch-0 cbranch-no" title="branch not covered" >changeAddress </span>: original.address;
    ...
    const permutedOutputs = Permutation.fromFunction(convertedOutputs, <span class="fstat-no" title="function not covered" >(a</span>, b) =&gt; {
...
<span class="cstat-no" title="statement not covered" >        return outputComparator(a.script, aValue, b.script, bValue);</span>
    }).map(o =&gt; o.output);
    return {
...
function convertInput(
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (inputAmounts) {
...
&nbsp;
function <span class="fstat-no" title="function not covered" >convertOpReturnOutput(</span>
    opReturnData: string,
): {
    output: Output,
    script: Buffer,
} {
    const opReturnDataBuffer = <span class="cstat-no" title="statement not covered" >Buffer.from(opReturnData, 'hex');</span>
    const output = <span class="cstat-no" title="statement not covered" >{</span>
        opReturnData: opReturnDataBuffer,
    };
    const script = <span class="cstat-no" title="statement not covered" >BitcoinJsScript.nullData.output.encode(opReturnDataBuffer);</span>
<span class="cstat-no" title="statement not covered" >    return {</span>
        output,
        script,
    };
...
function convertOutput(
    ...
    const output: Output = isChange ? <span class="branch-0 cbranch-no" title="branch not covered" >{</span>
