...
<html lang="en">
<head>
    <title>Code coverage report for coinselect-lib/outputs/split.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">coinselect-lib/outputs</a> split.js
    </h1>
    <div class='clearfix'>
...
function filterCoinbase(utxos, minConfCoinbase) {
    return utxos.filter((utxo) =&gt; {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (utxo.coinbase) {
<span class="cstat-no" title="statement not covered" >            return utxo.confirmations &gt;= minConfCoinbase;</span>
        }
        return true;
    });
}
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (feeRateBigInt.isNaN() || !feeRateBigInt.isInteger()) return {};
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (outputs.length === 0) return FEE_RESPONSE;
&nbsp;
    const inAccum = utils.sumOrNaN(utxos);
    <span class="missing-if-branch" title="if path not taken" >I</span>if (inAccum.isNaN()) return FEE_RESPONSE;
    const outAccum = utils.sumOrNaN(outputs, true);
    const remaining = inAccum.minus(outAccum).minus(new BigNumber(fee));
    <span class="missing-if-branch" title="if path not taken" >I</span>if (remaining.comparedTo(new BigNumber(0)) &lt; 0) return FEE_RESPONSE;
&nbsp;
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!outputs.every(x =&gt; x.value !== undefined
        || (splitValue.comparedTo(new BigNumber(dustThreshold)) &gt; 0))) <span class="cstat-no" title="statement not covered" >return FEE_RESPONSE;</span>
&nbsp;
