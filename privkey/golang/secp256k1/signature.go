...
func SignCompact(key *PrivateKey, hash []byte, isCompressedKey bool) []byte {
	...
	fmt.Println("recovery code ", pubKeyRecoveryCode)
	...
	s.PutBytes(&sBytes)
	rbytes := r.Bytes()
	fmt.Println("r bytes ", hex.EncodeToString(rbytes[:]))
	sbytes := s.Bytes()
	fmt.Println("s bytes ", hex.EncodeToString(sbytes[:]))
...
func signRFC6979(privateKey *PrivateKey, hash []byte) (*Signature, byte) {
	...
	for iteration := uint32(0); ; iteration++ {
		...
		if s.IsOverHalfOrder() {
			sbytes := s.Bytes()
			fmt.Println("s before negate ", hex.EncodeToString(sbytes[:]))
			s.Negate()
			sbytes = s.Bytes()
			fmt.Println("s after negate ", hex.EncodeToString(sbytes[:]))
